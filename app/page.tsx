    ...
    <div className="space-y-10 pb-16 md:space-y-14">
      <section className="grid gap-5 md:grid-cols-5">
        <FadeIn delay={0.1}>
          <div className="relative overflow-hidden rounded-2xl border border-border/60 bg-card p-8 shadow-sm md:col-span-3">
            <Badge className="mb-5 rounded-full border-primary/20 bg-primary/10 text-xs font-medium text-primary">
      ...
      <RevealSection>
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {stats.map((stat) => (
            <div key={stat.label} className="rounded-2xl border border-border/60 bg-card p-6 text-center shadow-sm">
      ...
      <RevealSection>
        <div className="rounded-2xl border border-border/60 bg-card p-8 shadow-sm">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Technologies I work with regularly.</p>
          <div className="mt-6 grid grid-cols-3 gap-4 sm:grid-cols-4 md:grid-cols-6">
            {skills.slice(0, 12).map((item) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="group flex flex-col items-center gap-2 rounded-xl border border-border/60 bg-muted/30 p-4 text-center transition-all hover:-translate-y-1 hover:border-primary/30 hover:shadow-sm"
              >
                ...
                <span className="text-xs font-medium text-foreground/80 group-hover:text-foreground">{item.name}</span>
              </a>
            ))}
          </div>
        </div>
          ...
          <div className="mt-8 space-y-6">
            {timeline.map((item, idx) => (
              <div key={`${item.company}-${item.period}`} className="relative pl-14">
                <div className="absolute left-0 top-0 flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 text-sm font-bold text-primary ring-4 ring-background">
            ...
            <div className="rounded-xl border border-border/60 bg-muted/30 p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {quickInfo.coreSkills.map((s) => (
                  <span key={s} className="rounded-md border border-primary/20 bg-primary/5 px-2.5 py-1 text-xs font-medium text-primary">
                    {s}
                  </span>
                ))}
              </div>
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-8 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">A selection of recent work.</p>
            </div>
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {projects.slice(0, 3).map((project) => (
              <Link href={project.preview} target="_blank" key={project.name}
                className="group overflow-hidden rounded-xl border border-border/60 bg-muted/30 transition-all hover:-translate-y-1 hover:border-primary/30 hover:shadow-md"
              >
                ...
                <div className="p-4">
                  ...
                  <span className="mt-3 inline-flex items-center gap-1 text-sm font-medium text-primary transition-colors">
